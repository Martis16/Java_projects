...
public class ListKTU<E extends Comparable<E>>
 ...
 /////////////////////////////////////////////////////////////////////////////////
                    ...
                    public E set(int k,  E e){
                        ...
                        for (Node <E> d = first; d != null; d = d.next)
                    {
                        if (first.findNode(k) == d)
                        {
                            E temp = e;
                            d.element = temp;
                            temp = d.element;
		...
		for (;;) {
			...
			if (jauGerai) {
				return;
                  ...
                  public void minMax()
        {
            for(Node<E> f1=first; f1!=null;f1=f1.next)
            {
                Node<E> max = f1;
                for(Node<E>f2=f1; f2!=null; f2=f2.next)
                if(f2.element.compareTo(max.element)>0)
                    max=f2;
                E element = f1.element;
                f1.element=max.element;
                max.element=element;
            }
            
        }
            public void minMax(Comparator<E> comparator)
        {
            for(Node<E> f1=first; f1!=null;f1=f1.next)
            {
                Node<E> max = f1;
                for(Node<E>f2=f1; f2!=null; f2=f2.next)
                if(comparator.compare(f2.element,max.element)>0)
                    max=f2;
                E element = f1.element;
                f1.element=max.element;
                max.element=element;
