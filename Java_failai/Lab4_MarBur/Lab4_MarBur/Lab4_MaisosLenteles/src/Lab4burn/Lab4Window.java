...
public class Lab4Window extends JFrame implements ActionListener{
    ...
    private static final ResourceBundle MESSAGES = ResourceBundle.getBundle("Lab4burn.Plmessages");
    ...
    private PlayerMenu menus;
    
     private HashType ht = HashType.DIVISION;
    private MapADTx<String, Player> map;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private final PlayerGamyba booksCreator = new PlayerGamyba();
    ...
    public void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Player[] booksArray = booksCreator.createAndSellBooks(sizeOfGenSet, sizeOfInitialSubSet);
            for (Player c : booksArray) {
                map.put(
    ...
    public void mapAdd() throws MyException {
        Player c = booksCreator.sellBooks();
        map.put(
                booksCreator.getBooksIdFromDatabase(),// Raktas
                c);
        table.setModel(new TableModel(map.getModelList(panParam1.getTfOfTable().get(5).getText()),
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectedIndex()) {
            case 0:
                map = new MapKTUx<>(new String(), new Player(), initialCapacity, loadFactor, ht);
                break;
            case 1:
                map = new PlayerMapX<>(new String(), new Player(), initialCapacity, loadFactor, ht);
                break;
            // ...
