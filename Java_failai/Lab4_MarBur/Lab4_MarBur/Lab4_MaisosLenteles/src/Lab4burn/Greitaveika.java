...
public class Greitaveika {
    ...
    private final MapKTUx<String, Player> booksMap
            = new MapKTUx(new String(), new Player(), 10, 0.75f, HashType.DIVISION);
    private final MapKTUx<String, Player> booksMap2
            = new MapKTUx(new String(), new Player(), 10, 0.25f, HashType.DIVISION);
    
    private final PlayerMapX<String, Player> booksMapCustom
            = new PlayerMapX(new String(), new Player(), 10, 0.75f, HashType.DIVISION);
    private final PlayerMapX<String, Player> booksMap2Custom
            = new PlayerMapX(new String(), new Player(), 10, 0.25f, HashType.DIVISION);
    ...
    public void mapKTUTest() throws InterruptedException {
        try {
            ...
            for (int k : TESTED_AMOUNTS) {
                Player[] bookArray = PlayerGamyba.createBooks(k);
                String[] booksIdArray = PlayerGamyba.createBooksIds(k);
                booksMap.clear();
                booksMap2.clear();
                tk.startAfterPause();
                tk.start();
    ...
    public void books_MapTest() throws InterruptedException {
        try {
            ...
            //chainsSizes.add("   kiekis      " + TEST_NAMES[0] + "   " + TEST_NAMES[1]);
            for (int k : TESTED_AMOUNTS) {
                Player[] booksArray = PlayerGamyba.createBooks(k);
                String[] booksIdArray = PlayerGamyba.createBooksIds(k);
