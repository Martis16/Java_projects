...
import Lab3Burneika.Player;
import Lab3Burneika.ZaidGamyba;
...
import Lab3Burneika.GreitaveikosTyrimas;
...
public class Lab3Window extends JFrame implements ActionListener {
    ...
    private SortedSetADTx<Player> zaidSet;
...
//                KsSwing.setFormatStartOfLine(false);
    ...
    public void treeGeneration(String filePath) throws MyException {
        ...
        if (filePath == null) {
            autoArray = ZaidGamyba.generuotiIrIsmaisyti(sizeOfGenSet, sizeOfInitialSubSet, coef);
            panParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            zaidSet.load(filePath);
            autoArray = new Player[zaidSet.size()];
            int i = 0;
            for (Object o : zaidSet.toArray()) {
                autoArray[i++] = (Player) o;
            }
        ...
        zaidSet.clear();
        for (Player a : autoArray) {
            zaidSet.add(a);
        }
        // Išvedami rezultatai
        // Nustatoma, kad eilutės pradžioje neskaičiuotų išvedamų eilučių skaičiaus
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, zaidSet.toVisualizedString(delimiter),
    ...
    private void treeAdd() throws MyException {
        Player auto = ZaidGamyba.gautiIsBazes();
        zaidSet.add(auto);
        panParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsSwing.setFormatStartOfLine(true);
    ...
    private void treeRemove() {
        ...
        if (zaidSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(zaidSet.size());
            Player auto = (Player) zaidSet.toArray()[nr];
            zaidSet.remove(auto);
    ...
    private void createHeadSet(){
        KsSwing.setFormatStartOfLine(true);
        System.out.print("\n\nAukstis "+zaidSet.height());        
    }
    private void containstree(){
    KsSwing.setFormatStartOfLine(true);
    Player a5 = new Player("Lionelis Messi 1987 170 70 9.5");
    
    if(zaidSet.contains(a5)==true)
        KsSwing.oun(taOutput, a5, MESSAGES.getString("msg10"));
    else
        KsSwing.oun(taOutput, a5, MESSAGES.getString("msg11"));
    KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void treeIteration() {
        ...
        if (zaidSet.isEmpty()) {
            ...
            KsSwing.oun(taOutput, zaidSet, MESSAGES.getString("msg8"));
        }
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectedIndex()) {
            case 0:
                zaidSet = new BstSetKTUx(new Player());
                break;
            case 1:
                zaidSet = new AvlSetKTUx(new Player());
                break;
            default:
                enableButtons(false);
